...
| **Anulación** | Transición a `cancelled` de un documento emitido, con motivo obligatorio. | Billing | No es borrar: lo emitido nunca se borra. Ante Hacienda un comprobante válido no se anula: se corrige con una nota de crédito (art. 9; pendiente de decisión, ADR 0007). |
...
| **Consecutivo fiscal** | Número de 20 dígitos que exige Hacienda: establecimiento (3; `001` = casa matriz), terminal (5), tipo de comprobante (2: `01` factura, `02` nota de débito, `03` nota de crédito, `04` tiquete...) y número (10), por establecimiento o terminal. FUENTE: borrador Anexo 1 v4.4, Nota 3. | fiscal (`fiscal.document_sequences`) | No es el número visible. |
| **Clave numérica** | Identificador de 50 dígitos del comprobante: `506` + fecha `ddmmaa` + identificación del emisor rellenada a 12 dígitos + consecutivo (20) + situación (`1` normal, `2` contingencia, `3` sin internet) + código de seguridad (8). FUENTE: borrador Anexo 1 v4.4, Notas 3 y 4.1. | fiscal | No es un UUID del sistema. |
