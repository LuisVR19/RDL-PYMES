// Command migrate aplica las migraciones goose de core y subscriptions.
//
// Uso: migrate <up|up-by-one|status|down> (MIGRATE_DATABASE_URL con el login platform_migrate, conexión directa o puerto 5432).
// Se niega a correr si la sesión no es platform_migrator: así ningún objeto queda a nombre de otro rol.
package main
...
import (
	...
	"rdl/platform-api/migrations"
)

const (
	expectedRole = "platform_migrator"
	historyTable = "core.goose_db_version"
)
...
func run() error {
	if len(os.Args) != 2 {
		return errors.New("uso: migrate <up|status|down>")
	}
	...
	if url == "" {
		// Modo sesión (5432): el `set role platform_migrator` que fija ALTER ROLE debe durar toda la conexión.
		var err error
		url, err = config.PoolerURL(os.Getenv("SUPABASE_URL"), os.Getenv("DB_POOLER_HOST"), "platform_migrate", config.PoolerSessionPort)
		if err != nil {
			return err
		}
	}
