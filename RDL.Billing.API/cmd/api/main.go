// Command api es el composition root de Billing API: configuración, wiring y arranque. Sin lógica de negocio.
package main
...
import (
	...
	"syscall"
	"time"
	_ "time/tzdata" // la imagen distroless no trae zoneinfo: la fecha de negocio usa la zona de la organización

	"rdl/billing-api/internal/adapters/auth"
	...
	"rdl/billing-api/internal/platform/telemetry"
...
func main() {
	if err := run(); err != nil {
		fmt.Fprintln(os.Stderr, "billing-api:", err)
		os.Exit(1)
	}
}
...
func run() error {
	...
	go func() {
		log.Info("billing-api escuchando", slog.String("addr", cfg.HTTP.Addr))
