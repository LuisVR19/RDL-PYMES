...
where service = 'billing' and organization_id = $1 and idempotency_key = $2 and expires_at < now();
...
values ('billing', $1, $2, sqlc.arg(request_hash)::text, now() + make_interval(secs => sqlc.arg(ttl_seconds)::bigint))
...
where service = 'billing' and organization_id = $1 and idempotency_key = $2;
...
where service = 'billing' and organization_id = sqlc.arg(organization_id) and idempotency_key = sqlc.arg(idempotency_key);
