...
func load(getenv func(string) string) (Config, error) {
	...
	cfg := Config{
		...
		ServiceName: r.optional("OTEL_SERVICE_NAME", "billing-api"),
		HTTP: HTTP{
			Addr:              r.optional("HTTP_ADDR", ":8081"),
	...
	// Lo normal es no definir DATABASE_URL: se arma con el host de Supavisor y el login billing_api.
	if cfg.DB.URL == "" && supabaseURL != "" {
		u, err := PoolerURL(supabaseURL, getenv("DB_POOLER_HOST"), "billing_api", PoolerTransactionPort)
		if err != nil {
